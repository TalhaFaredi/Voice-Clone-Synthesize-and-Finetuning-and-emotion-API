...
from src.Spanish_f5tts.Spanish_train.Spanish_utils.transcribe import create_spanish_metadata, spanish_transcribe_all
...
spanish_transcribe_routes = Blueprint('spanish_transcribe_routes', __name__)

@spanish_transcribe_routes.route('/es/transcribe/<project_name>', methods=['POST'])
def spanish_transcribe_and_prepare(project_name):
    ...
    try:
        ...
        logger.info(f"Calling spanish_transcribe_all for {len(saved_file_paths)} files in project '{project_name}'.")
        transcription_result_dict = spanish_transcribe_all(project_name, saved_file_paths, language, user_mode=False)
        ...
        logger.info(f"Calling create_spanish_metadata for project '{project_name}'. Deduced char_tokenizer={tokenizer_is_char_based}")
        prepare_result_dict, _ = create_spanish_metadata(project_name, ch_tokenizer=tokenizer_is_char_based)
